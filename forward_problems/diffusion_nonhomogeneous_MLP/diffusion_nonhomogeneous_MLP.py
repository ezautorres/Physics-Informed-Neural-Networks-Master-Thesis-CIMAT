...
Description
-----------
Solves a nonhomogeneous diffusion equation on the space-time domain
$[-1,1]\times[0,1]$ using a Physics-Informed Neural Network (PINN) with
Dirichlet boundary conditions and an initial condition.

Governing PDE:
$$
    u_{t}(x,t) - u_{xx}(x,t) = -e^{-t}\sin(\pi x) (1 - \pi^{2}), 
...
- Loss terms:
  - PDE residual $L_\mathrm{pde}$ from the diffusion operator (via automatic
  differentiation).
  - Initial condition residual $L_\mathrm{ic}$ enforcing $u(x,0) = \sin(\pi x)$.
  - Boundary residual $L_\mathrm{bc}$ enforcing $u(-1,t)=u(1,t)=0$.
...
-----
To train the model:
    $ python diffusion_nonhomogeneous_MLP.py
...
- Collocation points sampled uniformly in the rectangular domain $[-1,1]\times[0,1]$.
...
class DiffusionNonhomogeneousPinn(PinnCore):
    ...
    def analytical_solution(self, X: torch.Tensor) -> torch.Tensor:
        """
        Returns the analytical solution $u(x, t) = \exp{-t} \sin(\pi x)$ evaluated
        at input points X.
    ...
    def loss_PINN(
        ...
        for i in range(N_total):
            ...
            if region == 1:
                ...
                # Source term: -exp(-t)sin(πx)(1 - π²).
                f = (
                    - torch.exp(-xt[0, 1])
                    * torch.sin(torch.pi * xt[0, 0])
                    * (1 - torch.pi**2)
                )
            ...
            # ----------------------------------------------------------------------
                ...
                u_0 = torch.sin(torch.pi * xt[0, 0])
                loss_ic += (u - u_0).pow(2).squeeze()
...
if __name__ == "__main__":
    ...
    plot_loss(
        model_instance=diffusion_pinn, filename="loss_plot.png"
    )
