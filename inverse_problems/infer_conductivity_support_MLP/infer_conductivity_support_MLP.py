"""
infer_conductivity_support_MLP.py
-----------------------------------
Parametric Physics-Informed Neural Network (PINN) for the Conductivity
Equation in the Unit Disk (parameter R).
...
conductivity problem in the unit disk. The conductivity depends on a fixed
contrast parameter $\rho = 6$ and a variable interface radius $R. The
...
Implementation
--------------
- Class `ConductivityDiskParametric` inheriting from `PinnBase`.
- Conductivity function `lambda_fn` with fixed $\rho=6$ and variable $R$.
...
-----
To train the model:
    $ python conductivity_disk_parametric_MLP.py

Example:
>>> R = 0.725
>>> plot_solution_circle(
...     model_instance=conductivity_pinn,
...
- This script trains a **parametric PINN** where $R$ is the only parameter,
  with $\rho=6$ fixed. Visualization is shown for $R=0.725$.
"""
# Necessary libraries.
...
class InferringConductivitySupport(PinnBase):
    def __init__(self, **params):
        """
        Initializes the InferringConductivitySupport instance using the
        ...
        **params : dict
        ...
        super(InferringConductivitySupport, self).__init__(**params)
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        u_boundary = infer_R_pinn.pinn(X_boundary)
